...
// @version      2.3.1
// @description  按发帖时间排序的时间线视图，支持分类/标签筛选+拖拽排序，可折叠面板；标签面板默认折叠，筛选与折叠状态自动记忆，新帖提醒，重开不刷新
...
(function () {
  ...
  // ---------- 持久化偏好 ----------
  const PREF = {
    drawer: "timeline_drawer_open",
    panel: "timeline_panel_open",
    cats: "timeline_selected_categories",
    tags: "timeline_filter_tags",
    order: "timeline_cat_order",
  };
  ...
  function prefSet(key, value) {
    try { GM_setValue(key, JSON.stringify(value)); } catch (e) { /* 存储失败不影响使用 */ }
  }

  // 内部状态（重载去重 / 防抖 / 轮询 / 新帖计数 / 数据新鲜度）
  let pendingReload = false;
  let catReloadTimer = null;
  let pollTimer = null;
  let newTopicsCount = 0;
  let lastLoadedAt = 0;       // 最近一次成功拉取数据的时间戳
  const STALE_MS = 5 * 60 * 1000; // 缓存超过该时长视为过期，重开时重新拉取
  ...
  GM_addStyle(`
    ...
    .timeline-collapse-summary {
      font-size: 11px; color: var(--tertiary, #08c); max-width: 160px;
      white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
    }
    .timeline-drawer-new {
      width: auto; height: 30px; padding: 0 10px; border: 1px solid var(--danger, #e45735);
      border-radius: 15px; background: var(--danger-low, #fdeaea); color: var(--danger, #e45735);
      font-size: 12px; cursor: pointer; white-space: nowrap; transition: opacity .2s;
      display: none; align-items: center;
    }
    .timeline-drawer-new:hover { opacity: .85; }
  ...
  function createDrawer() {
    ...
    drawer.innerHTML = `
      <div class="timeline-drawer-header">
        ...
        <div class="timeline-drawer-actions">
          <button class="timeline-drawer-new" title="有新帖子，点击刷新" style="display:none"></button>
      ...
      <div class="timeline-collapse-panel" id="timeline-tag-panel">
        <div class="timeline-collapse-header" data-toggle="tag">
          ...
          <span class="timeline-collapse-summary"></span>
    ...
    drawer.querySelector(".timeline-drawer-new").addEventListener("click", () => loadTimelineTopics());
  ...
  function setupCollapsePanels(drawer) {
    drawer.querySelectorAll(".timeline-collapse-header").forEach(header => {
      header.addEventListener("click", () => {
        ...
        if (panel.id) prefSet(`${PREF.panel}:${panel.id}`, isOpen);
      });
    });
  }
  ...
  function saveCategoryPrefs() {
    prefSet(PREF.cats, [...selectedCategories]);
    prefSet(PREF.order, orderedCats);
  }

  // 分类切换时的防抖重载：连续点选只触发一次请求
  function scheduleCategoryReload() {
    if (catReloadTimer) clearTimeout(catReloadTimer);
    catReloadTimer = setTimeout(() => { catReloadTimer = null; loadTimelineTopics(); }, 250);
  }
  ...
  function updateTagCountEl() {
    ...
    // 面板折叠时，在标题行展示已选标签摘要
    const summary = document.querySelector("#timeline-tag-panel .timeline-collapse-summary");
    if (summary) {
      summary.textContent = filterTags.length > 0
        ? "#" + filterTags.slice(0, 2).join(" #") + (filterTags.length > 2 ? ` +${filterTags.length - 2}` : "")
        : "";
    }
  }
  ...
  async function loadTimelineTopics(retryCount = 0) {
    ...
    try {
      ...
      isLoading = false;
      // 加载期间又有新请求排队（如快速切换分类），结束后补一次
      if (pendingReload) { pendingReload = false; loadTimelineTopics(); }
    }
  }
  ...
  async function loadMoreTopics() {
    ...
    try {
      ...
      if (!catIds) {
        const data = await fetch(`/latest.json?order=created&page=${currentPage}`)
          .then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); });
        loadingEl?.remove();
  ...
  // ==================== 新帖轮询提醒 ====================
  function startNewTopicsPolling() {
    stopNewTopicsPolling();
    pollTimer = setInterval(async () => {
      if (!isDrawerOpen || isLoading || isLoadingMore) return;
      try {
        const res = await fetch("/latest.json?order=created");
        if (!res.ok) return;
        const data = await res.json();
        if (!data?.topic_list?.topics?.length) return;
        let cnt = 0;
        data.topic_list.topics.forEach(t => { if (!loadedTopicIds.has(t.id)) cnt++; });
        if (cnt > 0 && cnt !== newTopicsCount) {
          newTopicsCount = cnt;
          updateNewTopicsBadge();
        }
      } catch (e) { /* 静默失败，下轮再试 */ }
  ...
  function stopNewTopicsPolling() {
    if (pollTimer) { clearInterval(pollTimer); pollTimer = null; }
  }

  function updateNewTopicsBadge() {
    const el = document.querySelector(".timeline-drawer-new");
    if (!el) return;
    if (newTopicsCount > 0 && isDrawerOpen) {
      el.style.display = "inline-flex";
      el.textContent = `\u25CF ${newTopicsCount} 新`;
    } else {
  ...
  function openDrawer() {
    ...
    // 恢复上次的筛选状态（分类 / 标签 / 分类排序）
    const mains = getMainCategories();
    const savedCats = prefGet(PREF.cats, []);
    selectedCategories = new Set(Array.isArray(savedCats) ? savedCats.filter(c => mains.includes(c)) : []);
    const savedTags = prefGet(PREF.tags, []);
    filterTags = Array.isArray(savedTags) ? savedTags.filter(t => typeof t === "string") : [];
    const savedOrder = prefGet(PREF.order, []);
    const orderList = Array.isArray(savedOrder) ? savedOrder : [];
    orderedCats = [];
    const seen = new Set();
    orderList.forEach(c => { if (mains.includes(c) && !seen.has(c)) { orderedCats.push(c); seen.add(c); } });
    mains.forEach(c => { if (!seen.has(c)) { orderedCats.push(c); seen.add(c); } });
    ...
    // 标签面板：恢复上次折叠/展开状态（默认折叠，不再每次默认展开）
    ...
    const panelOpen = prefGet(`${PREF.panel}:timeline-tag-panel`, false);
    if (tagBody) tagBody.classList.toggle("open", panelOpen);
    if (tagArrow) tagArrow.classList.toggle("open", panelOpen);
    ...
    updateActiveTagsUI(tagContainer);
    updateTagCountEl();

    // 持久化打开状态，启动新帖轮询
    prefSet(PREF.drawer, true);
    startNewTopicsPolling();

    // 有新鲜缓存则直接复用（保留列表与滚动位置），过期或首次才重新拉取
    if (allTopics.length > 0 && Date.now() - lastLoadedAt < STALE_MS) {
      renderTopics();
    } else {
      loadTimelineTopics();
    }
  ...
  // ==================== 键盘事件 ====================
  // Esc 只负责关闭，抽屉关着时按 Esc 不会再误打开
  document.addEventListener("keydown", (e) => { if (e.key === "Escape" && isDrawerOpen) closeDrawer(); });

  // ==================== 启动 ====================
  createFloatButton();
  // 上次关闭时抽屉处于打开状态则自动恢复
  if (prefGet(PREF.drawer, false)) openDrawer();
  console.log("[时间线] 已加载 v2.3.1 - 标签面板默认折叠, 筛选/面板状态记忆, 新帖提醒, 重开不刷新");
})();
